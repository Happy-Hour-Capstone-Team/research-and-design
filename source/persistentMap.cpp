...
int main() {
  ...
  EnvEntry pair1{std::make_shared<std::pair<Token, std::any>>(
      std::make_pair(Token{"test1"}, 1))};
  EnvEntry pair2{std::make_shared<std::pair<Token, std::any>>(
      std::make_pair(Token{"test2"}, 2))};
  EnvEntry pair3{std::make_shared<std::pair<Token, std::any>>(
      std::make_pair(Token{"test3"}, 3))};
  std::vector<EnvEntry> pairs1{pair1, pair2};
  std::vector<EnvEntry> pairs2{pair1, pair2, pair3};
  for(EnvEntry entry : pairs1) std::cout << entry << '\n';
  for(EnvEntry entry : pairs2) std::cout << entry << '\n';
  return 0;
}